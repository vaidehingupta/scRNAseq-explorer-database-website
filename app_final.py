        ...
        # ── Dot plot ──────────────────────────────────────────────────────────
            ...
            cur.execute(f"""
                SELECT mc.Final_Cell_Type, mc.APOE_Genotype,
                    AVG(ef.value),
                    COUNT(ef.cell_id) / tot.total
                FROM expression_sub ef
                STRAIGHT_JOIN merged_cells mc ON mc.cell_id = ef.cell_id
                JOIN (
                    SELECT Final_Cell_Type, APOE_Genotype, COUNT(*) AS total
                    FROM merged_cells
                    GROUP BY Final_Cell_Type, APOE_Genotype
                ) tot ON tot.Final_Cell_Type = mc.Final_Cell_Type
                    AND tot.APOE_Genotype   = mc.APOE_Genotype
                {w}
